angular.module('neoviewApp').controller('staffController', [
  '$scope',
  '$state',
  'Restangular',
  '$stateParams',
  ...
  function(
    ...
    switch ($state.current.name) {
      ...
      case 'app.staffUser':
        $scope.newFlg = false;
        Restangular.one('user', $stateParams.id)
          .get({}, {})
          .then(
            function(userInfo) {
              $scope.user = userInfo.plain();
              $scope.user.camera =
                $scope.user.camera === null ? '' : $scope.user.camera;
              userCam = userInfo.camera;
              Restangular.one('getCamera')
                .get({}, {})
    ...
    var deleteFn = $rootScope.$on('DeleteUser', function(evt, userInfo) {
      Restangular.one('user', userInfo.userId)
        .remove()
        .then(
          function(res) {
            $state.reload();
