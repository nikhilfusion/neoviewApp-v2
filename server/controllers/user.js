...
module.exports = function(ws, io) {
  ...
  function test(arr1, arr2) {
    ...
    if (chkDltArray.length > 0 && chkDltArray[0].name) {
      ...
      db.all('SELECT * from users  WHERE camera=?', [deletedCam], function(
        err,
        userInfo
      ) {
        if (!err && userInfo.length > 0) {
          db.run('UPDATE users SET camera = ? WHERE id = ?', [
            '',
            userInfo[0].id
          ]);
  ...
  function sendSms(mobileNumber, otp) {
    const msg =
      'Please use code ' +
      otp +
      ' to complete your login to NeoView.  Team NeoView';
  ...
  this.login = function(req, res) {
    ...
    db.serialize(function() {
      db.all('SELECT * from users  WHERE email = ?', [reqInfo.email], function(
        err,
        rows
      ) {
        if (!err && rows.length > 0) {
          ...
          if (bcrypt.compareSync(reqInfo.password, rows[0].password)) {
            ...
            db.run(
              'UPDATE users SET conn_flg = ?, otp = ?, otpCreated = ? WHERE id = ?',
              [true, otp, new Date().getTime(), Number(rows[0].id)]
            );
  ...
  this.logout = function(req, res) {
    var reqDt = req.body;
    db.all('SELECT * from users  WHERE id=?', [Number(reqDt.id)], function(
      err,
      rows
    ) {
      if (rows.length > 0) {
        db.run('UPDATE users SET conn_flg = ? WHERE id = ?', [
          false,
          Number(rows[0].id)
        ]);
  ...
  this.signup = function(req, res) {
    ...
    if (reqDt.username && reqDt.email && (reqDt.role || reqDt.role == 0)) {
      ...
      db.serialize(function() {
        db.run(
          'CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, password TEXT, email TEXT, role INTEGER, camera TEXT, mobile TEXT, otp INTEGER, otpCreated DATETIME, conn_flg BOOLEAN)'
        );
        db.all('SELECT id from users WHERE  email=?', [reqDt.email], function(
          err,
          rows
        ) {
          if (!err) {
  ...
  this.getUser = function(req, res) {
    db.all('SELECT * from users  WHERE id=?', [Number(req.params.id)], function(
      err,
      userInfo
    ) {
      if (!err && userInfo.length > 0) {
  ...
  this.getCamera = function(req, res) {
    ...
    db.all('SELECT name FROM sqlite_master WHERE name= ?', ['users'], function(
      err,
      dt
    ) {
      if (!err) {
        if (dt.length === 0) {
          ...
          db.all('SELECT camera from users where camera IS NOT NULL', function(
            err,
            cameras
          ) {
            if (!err) {
  ...
  this.otpVerifie = function(req, res) {
    var otpDt = req.body;
    db.all('SELECT * from users  WHERE id=?', [Number(otpDt.userId)], function(
      err,
      userInfo
    ) {
      if (
        Number(userInfo[0].otp) === Number(otpDt.otp) &&
        new Date().getTime() - userInfo[0].otpCreated < oneDay
      ) {
        res.send(userInfo[0]);
      } else {
  ...
  this.resendOTP = function(req, res) {
    var reqInfo = req.body;
    db.all('SELECT * from users  WHERE id = ?', [reqInfo.userId], function(
      err,
      rows
    ) {
      var userInfo = rows[0],
      ...
      if (new Date().getTime() - userInfo.otpCreated < oneDay) {
        ...
        db.run('UPDATE users SET otp = ?, otpCreated = ? WHERE id = ?', [
          otp,
          new Date().getTime(),
          reqInfo.userId
        ]);
      }
      sendSms(userInfo.mobile, otp).then(
        function(smsRes) {
  ...
  function sendMail(userInfo, subject) {
    var mailOptions = {
      ...
      subject: subject,
      html:
        '<h2>This is Your NeoView Credentials</h2></br>' +
        '<p>Email : ' +
        userInfo.email +
        '</p></br>' +
        '<p>Password : ' +
